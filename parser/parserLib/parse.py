...
def date(date):
    date = date.lower()
    if date.lower() in ["по настоящее время", "currently"]:
        return None

    date = date.split()

    if len(date) == 2:
        date = datetime.strptime(f"{date[0]} {date[1]}", "%B %Y").date()
    elif len(date) == 3:
        date = datetime.strptime(f"{date[0]} {months.index(date[1]) + 1} {date[2]}", "%d %m %Y").date()
...
def contacts(page):
    phone_ = page.find("div", {"data-qa": "resume-contacts-phone"}).find("a").getText()
    email = page.find("div", {"data-qa": "resume-contact-email"}).find("a").getText()
    return {"email": email, "phone": phone_}


@get_optional_text
def personal_name(page):
    return page.find("h2", {"data-qa": "resume-personal-name"})
...
def position_salary(position_block):
    ...
    :return: dict
    """
    salary = position_block.find("span", {"data-qa": "resume-block-salary"})
    amount = None
    currency = None
...
def experiences(page, format="%Y-%m-%d"):
    """
    :param bs4.BeautifulSoup page: resume page
    ...
    if page is not None:
        ...
        for experience_item in page.findAll("div", {"class": "resume-block-item-gap"}):
            time_interval = experience_item.find("div", {
                "class": "bloko-column bloko-column_xs-4 bloko-column_s-2 bloko-column_m-2 bloko-column_l-2"})
            time_interval.div.extract()

            start, end = time_interval.getText().replace("\xa0", " ").split(' — ')

            item_position = experience_item.find("div", {"data-qa": "resume-block-experience-position"})
            ...
            # item_organization = experience_item.find("div", {"class": "bloko-column bloko-column_xs-4 bloko-column_s-6 bloko-column_m-7 bloko-column_l-10"});
            # item_organization = item_organization.find("div", {"class": "resume-block-container"});
            item_organization = experience_item.find("div", {"class": "bloko-text bloko-text_strong"}).getText();
...
def resume(page):
    ...
    return {
        ...
        "birth_date": str(date(birth_date(page))),
        "gender": gender(page),
        "area": area(page),
